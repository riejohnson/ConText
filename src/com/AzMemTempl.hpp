...
/*-----------------------------------------------------*/
/* AzBaseArray is outdated.  Use AzBaseArr instead.    */
/*-----------------------------------------------------*/
template<class T, class Int=int>
...
public:
  ...
  inline T *array() { return a; }
protected:
  void err(const char *s1, const char *s2, const char *s3) const {
    throw new AzException(s1, s2, s3); 
  }
};
...
/*-----------------------------------------------------*/
template<class T, class Int=int>
class AzBaseArr  /* expandable array of base type that can be copied by memcpy */
{
protected:
  T *a; 
  Int num;
public:
  AzBaseArr(Int inp_num) : a(NULL), num(0) {
    alloc(inp_num); 
  }
  AzBaseArr(Int inp_num, T initval) : a(NULL), num(0) {
    alloc(inp_num, initval); 
  }
  AzBaseArr() : a(NULL), num(0) {}
  ~AzBaseArr() {
    AzMemTools<Int>::free(&a); 
  }
  void free_alloc(Int inp_num, const char *eyec="AzBaseArr::free_alloc", const char *msg="") {
    free(); alloc(inp_num, eyec, msg); 
  }    
  void alloc(Int inp_num, 
             const char *eyec="AzBaseArr::alloc", const char *msg="") {
    if (a != NULL) err("check failed", eyec, msg); 
    num = inp_num; 
    if (num > 0) AzMemTools<Int>::alloc(&a, num, eyec, msg); 
  }
  void free_alloc(Int inp_num, T initval, const char *eyec="AzBaseArr::alloc(num,val)", const char *msg="") {  
    free(); alloc(inp_num, initval, eyec, msg); 
  }
  void alloc(Int inp_num, T initval, 
             const char *eyec="AzBaseArr::alloc(num,val)", const char *msg="") {
    alloc(inp_num, eyec, msg); 
    for (int ix = 0; ix < num; ++ix) a[ix] = initval; 
  }  
  void realloc(Int new_num, 
               const char *eyec="AzBaseArr::realloc", const char *msg="") {
    AzMemTools<Int>::realloc_base(&a, num, new_num, eyec, msg); 
    num = new_num;
  }
  void transfer_from(AzBaseArr<T,Int> *inp, 
                     const char *eyec="AzBaseArr::transfer_from", const char *msg="") 
  {
    AzMemTools<Int>::free(&a); num = 0; /* free this data */
    a = inp->a; inp->a = NULL;     /* transfer data from inp to this */
    num = inp->num; inp->num = 0;  
  }
  void free(const char *eyec="AzBaseArr::free", const char *msg="") {
    if (a != NULL) {
      AzMemTools<Int>::free(&a); num = 0; 
    }
  }
  const T *point() const { return a; }
  T *point_u() { return a; } /* use this with caution */
  inline Int size() const { return num; }
  T operator[](Int index) const { 
    if (index < 0 || index >= num) err("AzBaseArr []", "index is out of range", ""); 
    return a[index]; 
  }
  T operator()(Int index, T val) {
    if (index < 0 || index >= num) err("AzBaseArr ()", "index is out of range", "");
    a[index] = val;  
    return val; 
  }
...
protected:
  ...
  Int num;
public:
  AzObjPtrArr(Int inp_num) : a(NULL), num(0) {
    alloc(inp_num); 
  }
  AzObjPtrArr() : a(NULL), num(0) {}
  ~AzObjPtrArr() { free(); }
  void free_alloc(Int inp_num, const char *eyec="AzObjPtrArr::free_alloc", const char *msg="") {
    free(); alloc(inp_num, eyec, msg); 
  }    
  void alloc(Int inp_num, const char *eyec="AzObjPtrArr::alloc", const char *msg="") {
    if (a != NULL) err("check failed", eyec, msg); 
    num = inp_num; 
    if (num <= 0) return; 
    AzMemTools<Int>::alloc(&a, num, eyec, msg); 
    for (int ix = 0; ix < num; ++ix) a[ix] = NULL; 
  }
  void free(const char *eyec="AzObjPtrArr::free", const char *msg="") {
    if (a == NULL) return; 
    for (int ix = 0; ix < num; ++ix) if (a[ix] != NULL) delete a[ix];  
    AzMemTools<Int>::free(&a); num = 0; 
  }
  inline Int size() const { return num; }
  const T *operator[](int index) const { 
    if (index < 0 || index >= num) err("AzObjPtrArr []", "index is out of range", ""); 
    if (a[index] == NULL) err("AzObjPtrArr []", "null element was referred.", ""); 
    return a[index]; 
  }
  ...
  void set(int index, T *ptr) {
    if (index < 0 || index >= num) err("AzObjPtrArr::set", "index is out of range", "");
    if (a[index] != NULL) err("AzObjPtrArr::set", "There is already a pointer in the designated slot.", ""); 
    a[index] = ptr; 
  }  
  void check_if_ready(const char *eyec, const char *msg="") const {
    for (int ix = 0; ix < num; ++ix) if (a[ix] == NULL) err("AzObjPtrArr::check_if_ready", eyec, msg);  
  }
protected:
  void err(const char *s1, const char *s2, const char *s3) const {
    throw new AzException(s1, s2, s3); 
