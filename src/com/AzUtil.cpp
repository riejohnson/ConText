...
void AzFile::open(const char *inp_flags) {
  ...
  AzX::throw_if(str_fn == NULL || str_fn->length() <= 0, eyec, "No filename"); 
  if (strcmp(flags, "X") == 0) flags = "rb"; /* for compatibility .. */
  AzX::throw_if((fp = fopen(pointFileName(), flags)) == NULL, AzFileIOError, eyec, pointFileName(), "fopen"); 
}
...
void AzFile::close(bool doCheckError) {
  ...
  if (fp != NULL) {
    AzX::throw_if(fclose(fp) != 0 && doCheckError, AzFileIOError, eyec, pointFileName(), "fclose"); 
    fp = NULL; 
  }
}
...
AZint8 AzFile::size() {
  ...
  /*-----  seek to eof  -----*/  
  AzX::throw_if(fseek(fp, 0, SEEK_END) != 0, AzFileIOError, eyec, pointFileName(), "seek to end"); 
  AZint8 size = ftell(fp); 
  AzX::throw_if (size == -1L, AzFileIOError, eyec, pointFileName(), "ftell"); 

  /*-----  seek it back  -----*/
  AzX::throw_if (fseek(fp, offs, SEEK_SET) != 0, AzFileIOError, eyec, pointFileName(), "seek"); 
  return size; 
}
...
int AzFile::gets(AzByte *buff, int buffsize) {
  ...
  if (fgets((char *)buff, buffsize, fp) == NULL) {
    if (feof(fp)) return 0; 
    AzX::throw_if(true, AzFileIOError, eyec, pointFileName(), "fgets"); 
  }
...
void AzFile::seekReadBytes(AZint8 offs, AZint8 len, void *buff) {
  ...
  if (len > 0) {
    AzX::throw_if(fread(buff, (size_t)len, 1, fp) != 1, AzFileIOError, eyec, pointFileName(), "fread"); 
  }
}
...
void AzFile::seekReadBytes(AZint8 offs, AZint8 sz, AZint8 count, void *buff) {
  ...
  if (sz > 0 && count > 0) {
    AzX::throw_if(fread(buff, (size_t)sz, (size_t)count, fp) != count, AzFileIOError, eyec, pointFileName(), "fread"); 
  }
}
...
void AzFile::checkBinMarker() {
  ...
  AzX::throw_if(int_val != AZ_BIN_MARKER_INT || dbl_val != AZ_BIN_MARKER_DBL, 
                AzInputError, "AzFile::checkBinMarker", 
...
/* static */
/*------------------------------------------------------------------*/
bool AzFile::scan(const char *fn, 
                  int buff_size, 
                  AzIntArr *ia_line_len, 
                  bool do_throw_if_null, 
                  int max_line_num) { /* -1: means don't care; added 06/15/2013 */
  AzX::throw_if_null(ia_line_len, "AzFile::scan", "ia_line_len");                   
  bool is_there_null = false; 
  ...
  ia_line_len->reset(); ia_line_len->prepare(1000000); 
  bool do_exit = false; 
  int remain = 0; 
  for ( ; ; ) {
    if (do_exit) break; 
    ...
    if (memchr(buff, '\0', len) != NULL) {
      is_there_null = true; 
      AzBytArr s(": A null character (\\0) was detected in the text file: "); s << file.pointFileName() << " .  "; 
      s << "Remove or replace null characters and retry."; 
      AzX::throw_if(do_throw_if_null, AzInputError, "AzFile::scan", s.c_str()); 
    }
    
    const char *wp = (char *)buff, *buff_end = (char *)(buff+len); 
    while(wp < buff_end) {
      const char *next_wp = (char *)memchr(wp, '\n', buff_end - wp); 
      if (next_wp == NULL) {
...
int az_compare_IIFarr_Float_D_Int1Int2_A(const void *v1, const void *v2); 
int az_compare_IIFarr_Float_A_Int1Int2_D(const void *v1, const void *v2); 
...
int AzIIFarr::bsearch_Float(double key, bool isAscending) const {
  ...
  if (isAscending) ptr = (AzIIFarrEnt *)bsearch(&key, ent, ent_num, sizeof(ent[0]), 
                                                az_compare_IIFarr_Float_A); 
  else             ptr = (AzIIFarrEnt *)bsearch(&key, ent, ent_num, sizeof(ent[0]), 
...
double AzIIFarr::sum_Fval() const {
  double sum = 0; 
  for (int ix = 0; ix < ent_num; ++ix) sum += ent[ix].val; 
  return sum; 
}

/*------------------------------------------------------------------*/
bool AzIIFarr::isSame(const AzIIFarr *iifq) const {
  AzX::throw_if_null(iifq, "AzIIFarr::isSame"); 
  if (iifq->ent_num != ent_num) return false; 
  for (int ix = 0; ix < ent_num; ++ix) {
...
int AzIIFarr::find(int int1, int int2, int first_ix) const {
  for (int ix = first_ix; ix < ent_num; ++ix) {
    if (ent[ix].int1 == int1 && ent[ix].int2 == int2) return ix; 
  }
  return AzNone; 
}
...
double AzIIFarr::findMin(int *out_idx) const {
  if (ent_num == 0) {
    if (out_idx != NULL) *out_idx = AzNone; 
    return -1; 
  }
  double min_val = ent[0].val; 
  int min_idx = 0; 
  for (int ix = 1; ix < ent_num; ++ix) {
    if (ent[ix].val < min_val) {
      min_idx = ix; 
      min_val = ent[ix].val; 
    }
  }
  if (out_idx != NULL) *out_idx = min_idx; 
  return min_val; 
}
...
double AzIIFarr::findMax(int *out_idx) const {
  ...
  if (out_idx != NULL) *out_idx = max_idx; 
  return max_val; 
}

/*------------------------------------------------------------------*/
void AzIIFarr::_realloc(int req) { /* 6/10/2017 */
  const char *eyec = "AzIIFarr::_realloc"; 
  AzX::throw_if(req<0, eyec, "overflow? negative memsize requested."); 
  if (a.size() > req) return; 
  int num_max = req; 
  int sz = 1024*1024*10;
  if      (num_max <= 1) num_max = 32; 
  else if (num_max < sz) num_max *= 2; 
  else                   num_max += sz; 
  AzX::throw_if(num_max<0, eyec, "overflow"); 
  a.realloc(&ent, num_max, eyec, "ent"); 
}

/*-------------------------------------------------------------*/
void AzIIFarr::concat(const AzIIFarr *iifq2) {
  const char *eyec = "AzIIFarr::concat"; 
  if (iifq2 == NULL || iifq2->ent_num <= 0) return; 
...
int AzIIFarr::getNum(double req_val) const {
  int count = 0; 
  for (int ix = 0; ix < ent_num; ++ix) if (ent[ix].val == req_val) ++count; 
  return count; 
}
...
void AzIIFarr::concat(const AzIIFarr *iifq2, double req_val) {
  ...
  int new_num = ent_num + iifq2->ent_num; 
  _realloc(new_num); 
  for (int ix = 0; ix < iifq2->ent_num; ++ix) {
...
/*-------------------------------------------------------------*/
void AzIIFarr::update(int idx, int int1, int int2, double val) {
...
/*-------------------------------------------------------------*/
void AzIIFarr::write(AzFile *file) const {
...
/*-------------------------------------------------------------*/
void AzIIFarr::reset(int num, int int1, int int2, double val) {
  reset(); prepare(num); 
  for (int ex = 0; ex < num; ++ex) put(int1, int2, val); 
}
...
void AzIIFarr::prepare(int num) {
  ...
  if (num >= ent_num && num != ent_num_max) {
    ent_num_max = num; 
...
void AzIIFarr::cut(int new_num) {
  if (new_num < 0 || ent_num <= new_num) return; 
  ent_num = new_num; 
  return; 
}
...
/*-------------------------------------------------------------*/
void AzIIFarr::insert(int index, int int1, int int2, double fval) {
  AzX::throw_if(index < 0 || index > ent_num, "AzIIFarr::insert", "invalid index"); 

  /*---  append to the end to allocate memory if necessary  ---*/
  put(int1, int2, fval); 
  if (index == ent_num - 1) return; /* already in position */

  AzIIFarrEnt my_ent = ent[ent_num-1]; 

  /*---  make room for [index]  ---*/
  for (int ex = ent_num - 2; ex >= index; --ex) ent[ex+1] = ent[ex]; 
...
double AzIIFarr::get(int idx, int *int1, int *int2) const {
  AzX::throw_if (idx < 0 || idx >= ent_num, "AzIIFarr::get", "out of range"); 
  if (int1 != NULL) *int1 = ent[idx].int1; 
  if (int2 != NULL) *int2 = ent[idx].int2; 
  return ent[idx].val; 
}
...
void AzIIFarr::squeeze_Sum() {
  ...
  for (int ex = 0; ex < ent_num; ) {
    ...
    for (++ex; ex < ent_num; ++ex) {
      if (ent[ex].int1 != ent[ex1].int1 || ent[ex].int2 != ent[ex1].int2) break; 
...
void AzIIFarr::squeeze_Int1_Max() {
  ...
  for (int ex = 0; ex < ent_num; ) {
    if (ex != ex1) ent[ex1] = ent[ex]; 
    for (++ex; ex < ent_num; ++ex) {
      if (ent[ex].int1 != ent[ex1].int1) break; 
      if (ent[ex].val > ent[ex1].val) ent[ex1] = ent[ex]; 
    }
    ++ex1; 
  }
  ent_num = ex1; 
...
void AzIIFarr::sort_IntInt(bool isAscending) {
  if (ent_num <= 1) return; 
  if (isAscending) qsort(ent, ent_num, sizeof(ent[0]), az_compare_IIFarr_IntInt_A); 
  else             qsort(ent, ent_num, sizeof(ent[0]), az_compare_IIFarr_IntInt_D); 
}

/*-------------------------------------------------------------*/
void AzIIFarr::sort_Int2Int1(bool isAscending) {
  if (ent_num <= 1) return; 
  if (isAscending) qsort(ent, ent_num, sizeof(ent[0]), az_compare_IIFarr_Int2Int1_A); 
  else             qsort(ent, ent_num, sizeof(ent[0]), az_compare_IIFarr_Int2Int1_D); 
}

/*-------------------------------------------------------------*/
void AzIIFarr::sort_Float(bool isAscending) {
  if (ent_num <= 1) return; 
  if (isAscending) qsort(ent, ent_num, sizeof(ent[0]), az_compare_IIFarr_Float_A); 
  else             qsort(ent, ent_num, sizeof(ent[0]), az_compare_IIFarr_Float_D); 
}

/*-------------------------------------------------------------*/
void AzIIFarr::sort_FloatInt1Int2(bool is_float_ascending, bool is_int_ascending) {
  if (ent_num <= 1) return; 
  if (is_float_ascending) {
    if (is_int_ascending) qsort(ent, ent_num, sizeof(ent[0]), az_compare_IIFarr_FloatInt1Int2_A);
    else                  qsort(ent, ent_num, sizeof(ent[0]), az_compare_IIFarr_Float_A_Int1Int2_D);
  }
  else {
    if (is_int_ascending) qsort(ent, ent_num, sizeof(ent[0]), az_compare_IIFarr_Float_D_Int1Int2_A);
    else                  qsort(ent, ent_num, sizeof(ent[0]), az_compare_IIFarr_FloatInt1Int2_D);
  }
}  
...
void AzIIFarr::sort_FloatInt1Int2(bool isAscending) {
  if (ent_num <= 1) return; 
  if (isAscending) qsort(ent, ent_num, sizeof(ent[0]), az_compare_IIFarr_FloatInt1Int2_A); 
  else             qsort(ent, ent_num, sizeof(ent[0]), az_compare_IIFarr_FloatInt1Int2_D); 
}
...
void AzIIarr::read(AzFile *file) { 
  iifq.read(file); 
  AzX::throw_if(iifq.get_ent_type() != AzIIFarr_II, "AzIIarr::AzIIarr(file)", "type conflict");
}

/*------------------------------------------------------------------*/
void AzIIarr::reset(const int pairs[][2], int pair_num) {
  AzX::throw_if_null(pairs, "AzIIarr::reset(pairs)"); 
  reset(); prepare(pair_num); 
  for (int ix = 0; ix < pair_num; ++ix) {
...
void AzIFarr::read(AzFile *file) { 
  iifq.read(file); 
  AzX::throw_if(iifq.get_ent_type() != AzIIFarr_IF, "AzIFarr::_read", "type conflict");
}
...
void AzIntArr::transfer_from(AzIntArr *inp) {
  AzX::throw_if_null(inp, "AzIntArr::transfer_from"); 
...
void AzIntArr::initialize(int inp_num, int initial_value) {
  ...
  num = inp_num; 
  for (int ix = 0; ix < num; ++ix) ints[ix] = initial_value; 
}

/*------------------------------------------------------------------*/
void AzIntArr::put(int int_val) {
  int num_max = a.size(); 
  if (num >= num_max) _realloc(num+1); 
  ints[num] = int_val; 
  ++num; 
}
...
bool AzIntArr::toCount(AzIntArr *out_iq) const {
  AzX::throw_if_null(out_iq, "AzIntArr::toCount", "out_iq"); 
  bool isThereNegative = false; 
  int max_idx = max(); 
...
int AzIntArr::compare(const AzIntArr *iq1, const AzIntArr *iq2, 
                         int first_k) {
  AzX::throw_if_null(iq1, "AzIntArr::compare", "iq1"); 
  AzX::throw_if_null(iq2, "AzIntArr::compare", "iq2");   
  int cmp_num = MIN(iq1->num, iq2->num); 
  if (first_k > 0) cmp_num = MIN(cmp_num, first_k); 

  for (int ix = 0; ix < cmp_num; ++ix) {
  ...
  if (first_k > 0 && cmp_num == first_k) return 0; 
...
void AzIntArr::changeOrder(const AzIntArr *ia_old2new) {
  const char *eyec = "AzIntArr::changOrder"; 
  AzX::throw_if_null(ia_old2new, eyec, "ia_old2new"); 
  int old2new_num = ia_old2new->num; 
  const int *old2new = ia_old2new->ints; 
  AzX::throw_if(old2new_num != num, eyec, "number mismatch"); 

  AzIntArr ia_org(this); 
  for (int ix = 0; ix < num; ++ix) ints[ix] = -1; 
  for (int ix = 0; ix < num; ++ix) {
...
double AzIntArr::average() const {
  if (num <= 0) return 0; 
  double sum = 0; 
  for (int ix = 0; ix < num; ++ix)  sum += ints[ix]; 
...
void AzIntArr::initialize(const int *inp_ints, int inp_ints_num) {
  if (inp_ints_num <= 0) return;
  const char *eyec = "AzIntArr::initialize(array,num)"; 
  AzX::throw_if_null(inp_ints, eyec); 

  num = inp_ints_num; 
  if (num > 0) a.alloc(&ints, num, eyec, "ints"); 
...
/*------------------------------------------------------------------*/
void AzIntArr::write(AzFile *file) const {
  file->writeInt(num); 
...
void AzIntArr::fill(int num, int first_value) {
  ...
  for (int ix = 0; ix < num; ++ix) ints[ix] = first_value + ix; 
}

/*------------------------------------------------------------------*/
void AzIntArr::cut(int new_num) {
  if (num > new_num) num = new_num; 
}

/*------------------------------------------------------------------*/
void AzIntArr::unique() {
  if (ints == NULL || num <= 1) return; 

  qsort(ints, num, sizeof(ints[0]), az_compareInt_A);   

  int ix1 = 0; 
  for (int ix = 0; ix < num; ) {
    if (ix != ix1) ints[ix1] = ints[ix]; 
    ++ix; 
    for ( ; ix < num; ++ix) if (ints[ix] != ints[ix1]) break;
    ++ix1; 
  }
  num = ix1; 
}
...
/*------------------------------------------------------------------*/
void AzIntArr::_realloc(int req) {
  const char *eyec = "AzIntArr::_realloc"; 
  AzX::throw_if(req<0, eyec, "overflow? negative memsize requested."); 
  if (a.size() > req) return; 
  int num_max = req; 
  int sz = 1024*1024*10; /* 6/7/2017: changed from 1024*1024 */
  if      (num_max <= 1) num_max = 32; 
  else if (num_max < sz) num_max *= 2; 
  else                   num_max += sz; 
  AzX::throw_if(num_max<0, eyec, "overflow"); 
...
/*------------------------------------------------------------------*/
void AzIntArr::concat(const AzIntArr *ia2, int addval) {
  if (ia2 == NULL) return; 
  concat(ia2->ints, ia2->num, addval); 
}

/*------------------------------------------------------------------*/
void AzIntArr::concat(const int *ints2, int ints2_num, int addval) {
  if (ints2 == NULL || ints2_num == 0) return; 

  int new_num = num + ints2_num; 
  _realloc(new_num); 
  if (addval == 0) for (int ix = 0; ix < ints2_num; ++ix) ints[num + ix] = ints2[ix]; 
  else             for (int ix = 0; ix < ints2_num; ++ix) ints[num + ix] = ints2[ix]+addval;  
  
  num = new_num; 
}

/*------------------------------------------------------------------*/
void AzIntArr::remove(const int *rmv, int rmv_num) {
  const char *eyec = "AzIntArr::remove(rmv,#rmv)"; 
  if (rmv_num == 0) return; 
  AzX::throw_if_null(rmv, eyec); 
  int ox = rmv[0]; 
  for (int rx = 0; rx < rmv_num; ++rx) {
    AzX::throw_if(rx>0 && rmv[rx]<=rmv[rx-1], eyec, "input must be in the ascending order"); 
    int bx = rmv[rx]+1, ex = (rx+1<rmv_num) ? rmv[rx+1] : num; 
    for (int ix = bx; ix < ex; ++ix, ++ox) {
      ints[ox] = ints[ix]; 
    }
  }  
  num = ox; 
...
int AzIntArr::count_nonnegative(const int *ints, int num) {
  ...
  int count = 0; 
  for (int ix = 0; ix < num; ++ix) if (ints[ix] >= 0) ++count; 
  return count; 
}
...
void AzIntArr::remove(int idx) {
  ...
  for (int ix = idx + 1; ix < num; ++ix) ints[ix-1] = ints[ix]; 
  --num; 
}
...
void AzIntArr::insert(int idx, int val) {
  int num_max = a.size(); 
  if (num >= num_max) _realloc(num+1); 
  for (int ix = num - 1; ix >= idx; --ix) ints[ix+1] = ints[ix]; 
  ints[idx] = val; 
  ++num; 
}
...
void AzIntArr::sort(bool ascending) {
  if (num <= 1) return; 
  if (ascending) qsort(ints, num, sizeof(ints[0]), az_compareInt_A); 
  else           qsort(ints, num, sizeof(ints[0]), az_compareInt_D); 
}
...
int AzIntArr::count(int int_val) const {
  if (ints == NULL) return 0; 
  int out = 0; 
  for (int ix = 0; ix < num; ++ix) if (ints[ix] == int_val) ++out; 
  return out; 
}
...
void AzBytArr::transfer_from(AzBytArr *inp) {
  const char *eyec = "AzBytArr::transfer_from"; 
  AzX::throw_if_null(inp, eyec); 
  a.transfer_from(&inp->a, &bytes_long, &inp->bytes_long, eyec); 
...
/*-------------------------------------------------------------*/
void AzBytArr::concatInt(int number, int width, bool doFillWithZero) {
  stringstream s; 
...
/*-------------------------------------------------------------*/
void AzBytArr::concatInt8(AZint8 number, int width, bool doFillWithZero) {
  stringstream s; 
  if (width > 0) s.width(width); 
  if (doFillWithZero) s.fill('0'); 
  s << number; 
...
/*-------------------------------------------------------------*/
void AzBytArr::concatFloat(double number, int precision, bool doScientific) {
  stringstream s; 
...
/*--------------------------------------------------------*/
void AzBytArr::replace(const char *old_str, const char *new_str) {
  const char *eyec = "AzBytArr::replace"; 
  AzX::throw_if_null(old_str, eyec, "old"); 
  AzX::throw_if_null(new_str, eyec, "new"); 
  const char *inp_str = c_str(); 
  const char *ptr = strstr(inp_str, old_str); 
  if (ptr == NULL) return; 

  int old_str_len = Az64::cstrlen(old_str); 
  int front_len = Az64::ptr_diff(ptr - inp_str, eyec); 
...
void AzBytArr::strip() {
  ...
  for ( ; bp < data_end; ++bp) if (*bp > 0x20) break; 
  const AzByte *ep = data_end; 
  for ( ; ep-1 >= data; --ep) if (*(ep-1) > 0x20) break; 
  if      (bp >= ep) clear(); 
...
void AzBytArr::print(const AzByte *inp_bytes, int inp_bytes_len, const AzOut &out) {
  if (inp_bytes == NULL) return; 
  if (out.isNull()) return; 
  for (int ix = 0; ix < inp_bytes_len; ++ix) {
    *out.o << inp_bytes[ix]; 
...
void AzBytArr::replace(char old_char, char new_char) {
  AzX::throw_if(old_char == '\0', "AzBytArr::replace", "can't replace null char"); 
  if (bytes_long != NULL) {
    ...
    for (int ix = 0; ix < MIN(AZI_BYTES_SHORT_LEN, len); ++ix) {
...
void AzBytArr::initialize(const AzByte *bytes, int bytes_len) {
  ...
  if (bytes == NULL || bytes_len <= 0) return; 
...
void AzBytArr::update(int offs, AzByte new_val) {
  AzX::throw_if(offs < 0 || offs >= len, "AzBytArr::update", "out of range"); 
  if (bytes_long != NULL) bytes_long[offs] = new_val; 
  else                    bytes_short[offs] = new_val; 
}
...
/*-------------------------------------------------------------*/
void AzBytArr::write(AzFile *file) const {
  file->writeInt(len); 
  if (len > 0) {
    if (bytes_long != NULL) file->writeBytes(bytes_long, len + 1); 
    else                    file->writeBytes(bytes_short, len + 1); 
  }
}

/*-------------------------------------------------------------*/
int AzBytArr::compare(const AzBytArr *byteq2) const {
  AzX::throw_if_null(byteq2, "AzBytArr::compare"); 
  int bytes2_len; 
...
int AzBytArr::compare(const AzByte *bytes2, int bytes2_len) const {
  AzX::throw_if_null(bytes2, "AzBytArr::compare"); 
  int bytes1_len; 
...
bool AzBytArr::beginsWith(AzBytArr *byteq2) const {
  if (byteq2 == NULL) return true; 

  int bytes1_len, bytes2_len; 
  const AzByte *bytes1 = point(&bytes1_len); 
  const AzByte *bytes2 = byteq2->point(&bytes2_len); 
/*  if (bytes2_len == 0) true; 6/7/2017 */
  if (bytes1_len < bytes2_len) return false; 
  if (memcmp(bytes1, bytes2, bytes2_len) == 0) return true; 
  return false; 
}
...
bool AzBytArr::contains(const char *str) const {
  AzX::throw_if_null(str, "AzBytArr::cotains"); 
...
void AzBytArr::concat(const AzByte *inp_bytes, int inp_len) {
  ...
  if (inp_bytes == NULL || inp_len <= 0) return; 
...
AzByte *AzBytArr::point_u() {
  if (bytes_long != NULL) return bytes_long; 
  else                    return bytes_short; 
}
...
void AzBytArr::lwr() {
  char *c;
  if (bytes_long != NULL) c = (char *)bytes_long;
  else                    c = (char *)bytes_short; 
  while (*c) {
...
/*-------------------------------------------------------------*/
void AzTimeLog::print(const char *msg1, const char *msg2, const char *msg3, const AzOut &out) {
  if (out.isNull()) return; 
  _printTime(out); 
  AzPrint::write(out, msg1); 
  AzPrint::write(out, msg2); 
  AzPrint::writeln(out, msg3);   
  out.flush(); 
}
...
/*-------------------------------------------------------------*/
void AzClock::show(const AzOut &out, const char *str, clock_t clks, time_t tims) const {
  if (be_silent) return; 
  AzBytArr s(str); s << (double)tims << ", " << ((double)clks)/((double)CLOCKS_PER_SEC); 
